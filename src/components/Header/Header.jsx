...
export default function Header({ variant = 'room', roomId, activeTab }) {
  ...
  const toggleDark = () => { const next = !dark; applyTheme(next); setDark(next) }
  ...
  return (
    ...
    <header className="clov-hdr">
      ...
      <div className="clov-hdr-right">
        ...
        <button type="button" className="clov-hdr-icon-btn" onClick={toggleDark} title="테마 전환" aria-label="테마 전환">
          {dark ? (
