...
// 헤더 드롭다운 — 마스코트 미리보기 + 보유 코스튬 장착/해제. Mascot.jsx와 같은 규칙(장착
// 아이템에 이미지가 있으면 기본 스프라이트를 완전히 대체)을 그대로 미리보기에 재사용한다.
export default function MascotWardrobe({ onNavigateShop }) {
  ...
  useEffect(() => {
    ...
    const onDoc = (e) => { if (!e.target.closest('.clov-hdr-wardrobe-wrap')) setOpen(false) }
  ...
  return (
    <div className="clov-hdr-wardrobe-wrap">
      <button
        type="button"
        className={`clov-hdr-wardrobe-btn${open ? ' active' : ''}`}
        onClick={() => setOpen((v) => !v)}
        title="마스코트 꾸미기"
        aria-haspopup="menu"
      >
        ...
        <span className="clov-hdr-wardrobe-label">꾸미기</span>
      </button>
